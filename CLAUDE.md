...
- `src/App.tsx`가 오케스트레이션 계층이고, `src/screens/*`는 단계별 화면, `src/lib/*`는 런타임 및 비공개 초안 로직을 담당합니다.
- 이 저장소 안에는 백엔드나 영속 저장 계층이 없습니다. 사진 선택, 수동 모먼트 구성, 비공개 초안 완료는 현재 클라이언트 사이드 동작입니다.
...
- 현재 Toss 미니앱은 비공개 여정 초안만 만듭니다. 서버 publish, 토스 로그인 토큰 교환, 네이티브 계정 연결은 별도 제품/ADR 결정 전까지 추가하지 않습니다.

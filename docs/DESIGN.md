...
현재 코드 구현은 4단계 화면 흐름과 Toss 로그인/server auth 경계까지
반영했습니다. 실제 사진 업로드, 비공개 여정 생성 API 호출, 서버 저장 완료
상태는 ADR 0009의 저장 계약이 확정된 뒤 후속 구현에서 추가합니다.
...
| Journey basics | 여정 제목, 설명, 대표 사진을 짧게 정합니다. | `src/screens/JourneyBasicsScreen.tsx`, `src/lib/momentbook.ts` |
...
현재 코드의 화면 구조는 ADR 0009 기준의 4단계 비공개 저장 흐름을 따릅니다.

- 기본 진입점은 사진 선택 화면입니다. 공개 예시 탐색 화면은 기존 route로 유지합니다.
- 사진 선택, 여정 정보, 수동 모먼트 구성, 타임라인 미리보기, 비공개 저장 확인 화면이 분리되어 있습니다.
- 모먼트는 기본 3개 슬롯으로 시작하고 사용자가 필요할 때 추가할 수 있습니다.
- 모든 사진을 모먼트에 담지 않아도 타임라인으로 넘어갈 수 있으며, 남은 사진은 `unassignedPhotos`로 보존합니다.
- 저장 화면은 Toss `appLogin` authorization code와 서버 `/v2/auth/toss` 경계를 시도합니다.
- 비공개 여정 생성/사진 업로드 API 계약은 아직 연결하지 않았고, 공개 publish API도 호출하지 않습니다.
- 저장 완료 상태는 실제 비공개 저장 API가 연결된 뒤에만 사용자에게 보여야 합니다.
...
- 현재 `src/App.tsx`는 reducer 상태, 화면 접근 제어, hash/history 네비게이션, 저장 시도 상태를 함께 관리합니다.
- `src/lib/navigation.ts`는 화면 union, 순서, 메타데이터, 접근 규칙을 작게 유지합니다.
- `src/lib/appsInToss.ts`는 Toss SDK 접근을 얇은 어댑터 뒤에 둡니다.
- `src/lib/momentbook.ts`는 현재 비공개 여정 draft와 수동 모먼트 구성 로직을 담당합니다.
- `src/screens/*`는 가능한 한 표현 계층으로 유지합니다.
- 후속 업로드 구현에서는 서버 업로드 client와 retry/error 세부 상태를 화면 표현 계층과 분리합니다.
...
- 현재 코드는 완료 성공을 시뮬레이션하지 않습니다. 비공개 저장 API가 없으면 명시적 실패/대기 상태를 보여줍니다.
...
- 비공개 저장 API의 endpoint, request, response shape는 실제 계약을 확인한 뒤 반영합니다.
